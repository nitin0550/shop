...
from .models import SalerDetail, Product, ProductSize, SellerSlider, MyCart, WholeSaleProduct, category, Orders, WholeSaleProductOrders, SalerOTP
	...
	if request.user.is_authenticated:
		...
		if request.method == 'POST':
			...
			if get_otp:
				...
				usar = User.objects.get(username=get_usr)
				if int(get_otp) == SalerOTP.objects.filter(user = usar).last().otp:
					usar.is_active= True
					usar.save()
					messages.success(request, f'Account is Created for {usar.first_name}')
					return redirect('login')
				else:
					messages.warning(request, f'You Enterd a Wrong OTP')
					return render(request, 'saler/seller_signup.html', {'otp': True, 'usr': usar})
			...
			if form.is_valid():
				...
				if username.isdigit():
					...
					SalerDetail(user=usr,mobile=username).save()
					usr_otp = random.randint(100000, 999999)
					SalerOTP.objects.create(user = usr, otp = usr_otp)
					url = (f'https://2factor.in/API/V1/{settings.API_KEY}/SMS/{username}/{usr_otp}')
					response = requests.get(url)
					return render(request, 'saler/seller_signup.html', {'otp': True, 'usr': usr })
